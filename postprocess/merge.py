    ...
    for img in imgs:
        for y_small in range(0,256):
            for x_small in range(0,256):
                ...
                cur_pixel = mark_big[cur_y][cur_x][0]
                new_pixel_val = img[y_small][x_small][1]
                if cur_pixel == 0:
                    mark_big[cur_y][cur_x][0] = new_pixel_val
                else:
                    mark_big[cur_y][cur_x][0] = np.average([cur_pixel,new_pixel_val]) # 求平均
        i += 1
    return mark_big
